...
import { Box, Button, CircularProgress, FormControlLabel, Paper, Stack, Switch, Typography, IconButton, Tooltip } from '@mui/material';
...
export default function SakthiCVPage() {
	...
	const handleDownload = (type: 'cv' | 'coverletter') => {
		...
		link.href = url;
		link.download = type === 'coverletter' ? `Cover_Letter_${fileNameSuffix}.pdf` : `CV_${fileNameSuffix}.pdf`;
	...
	return (
		<Box sx={{ height: '100vh', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
			...
			>
				<FormControlLabel
					control={<Switch checked={isHeadhunterMode} onChange={e => setIsHeadhunterMode(e.target.checked)} color="primary" />}
					label={
				...
				<Stack direction="row" spacing={2} sx={{ alignItems: 'center' }}>
					...
					<Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
						...
						<Button variant="outlined" size="small" startIcon={<DownloadIcon />} onClick={() => handleDownload('coverletter')}>
							Download
						</Button>
						<Tooltip title="Open in New Tab">
							<IconButton onClick={() => window.open(getDownloadUrl('coverletter'), '_blank')} size="small" sx={{ ml: 0.5 }}>
					...
					<Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
						...
						<Button variant="contained" size="small" startIcon={<DownloadIcon />} onClick={() => handleDownload('cv')} disableElevation>
							Download
						</Button>
						<Tooltip title="Open in New Tab">
							<IconButton onClick={() => window.open(getDownloadUrl('cv'), '_blank')} size="small" sx={{ ml: 0.5 }}>
								<OpenInNewIcon fontSize="small" />
							</IconButton>
						</Tooltip>
					</Box>
