...
const registerFontFamily = async (family: string) => {
	const [regular, bold, italic] = await Promise.all([getGoogleFontUrl(family, 400), getGoogleFontUrl(family, 700), getGoogleFontUrl(family, 400, true)]);
...
export async function registerFonts() {
	await Promise.all([registerFontFamily('Inter'), registerFontFamily('Playfair Display'), registerFontFamily('Lora')]);
}
